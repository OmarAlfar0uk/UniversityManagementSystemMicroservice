...
public class SyncAssignmentHandler(IUnitOfWork uow, IHttpContextAccessor httpContextAccessor)
    : IRequestHandler<SyncAssignmentCommand, ProgressResponse>
{
    public async Task<ProgressResponse> Handle(SyncAssignmentCommand request, CancellationToken ct)
    {
        ...
        {
            progress.IsAssignmentSubmitted = true;
            progress.CompletionPercentage = CalculateCompletion(progress);
